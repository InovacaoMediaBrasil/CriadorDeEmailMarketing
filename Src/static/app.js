...
app.controller("form", function ($scope, $http) {
  $scope.availableTemplates = [
    ...
    {
      name: "Vitrine do Artesanato - Dicas de Artesanatos",
      data: "dicasdeartesanato",
    },
  ...
  $scope.$watch("selectTemplate", function (val) {
    if (!val || val.data == undefined) return;
    $("#error").hide();

    var r = Math.round(Math.random() * 1024);
    $http.get("data/" + val.data + "/schema.js?r=" + r).then(function (res) {
      $scope.schema = res.data;
    });
    $http.get("data/" + val.data + "/form.js?r=" + r).then(function (res) {
      $scope.form = res.data;
    });
    $http
      .get("data/" + val.data + "/template.html?r=" + r)
      .then(function (res) {
        $scope.template = Handlebars.compile(res.data);
      });
  });
});
