  ...
  angular.module("schemaForm").provider("sfPath", [
    function () {
      ...
      sfPath.normalize = function (data, quote) {
        return sfPath.stringify(
          Array.isArray(data) ? data : sfPath.parse(data),
          quote,
        );
      };
    ...
    function (sfPathProvider) {
      ...
      var builders = {
          ...
          for (var i = 0; i < nodes.length; i++) {
            ...
            if (!conf || conf === "") {
              ...
              for (var j = 0; j < attributes.length; j++) {
                if (
                  attributes[j].value &&
                  attributes[j].value.indexOf("$$value") !== -1
                ) {
        ...
        ngModelOptions: function (args) {
          if (
            args.form.ngModelOptions &&
            Object.keys(args.form.ngModelOptions).length > 0
          ) {
        ...
        transclusion: function (args) {
          var transclusions = args.fieldFrag.querySelectorAll(
            "[sf-field-transclude]",
          );
          ...
          var build = function (
            ...
            items.reduce(function (frag, f, index) {
              ...
              if (!field.replace) {
                ...
                var template =
                  templateFn(f, field) || templateFn(f, decorator["default"]);
    ...
    function ($compileProvider, sfPathProvider) {
          ...
          function (
            ...
            return {
              ...
              link: function (scope, element, attrs, sfSchema) {
                ...
                scope.$on(
                  "schemaFormPropagateNgModelController",
                  function (event, ngModel) {
                        ...
                        if (!scope.externalDestructionInProgress) {
                          ...
                          if (form.key && destroyStrategy !== "retain") {
                            ...
                            if (form.key.length > 1) {
                              obj = sfSelect(
                                form.key.slice(0, form.key.length - 1),
                                obj,
                              );
                            }
      ...
      var createManualDirective = function (type, templateUrl, transclude) {
        ...
        $compileProvider.directive(
          "sf" + angular.uppercase(type[0]) + type.substr(1),
          function () {
            return {
              restrict: "EAC",
              scope: true,
                    ...
                    var updateForm = function (val) {
                      if (angular.isDefined(val) && val !== form[name]) {
                        ...
                        if (
                          once &&
                          form.type &&
                          (form.key || angular.isUndefined(attrs.key))
                        ) {
                          scope.form = form;
                          once = false;
                        }
   ...
   */
      this.defineDecorator = function (name, fields) {
        ...
        angular.forEach(fields, function (field, type) {
          ...
          field.replace = angular.isDefined(field.replace)
            ? field.replace
            : true;
          ...
          for (var i = 0; i < rules.length; i++) {
            ...
            if (def) {
              ...
              if (
                ...
                angular.isObject(def.schema["x-schema-form"])
              ) {
        ...
        service.merge = function (
          ...
          return postProcessFn(
            form.map(function (obj) {
              ...
              if (
                obj.type === "checkbox" &&
                angular.isUndefined(obj.schema["default"])
              ) {
      ...
      return {
        ...
        link: {
          ...
          post: function (scope, element, attrs, sfSchema) {
            ...
            scope.showTitle = function () {
              return (
                scope.form && scope.form.notitle !== true && scope.form.title
              );
            };
            ...
            if (scope.form && scope.form.schema && scope.form.schema.items) {
              ...
              if (items.type && items.type.indexOf("object") !== -1) {
                ...
                if (
                  ...
                  empty = angular.isDefined(items["default"])
                    ? items["default"]
                    : empty;
        ...
        link: function (scope, element, attrs, formCtrl, transclude) {
          ...
          var internalRender = function (schema, form, merged) {
            ...
            for (var i = 0; i < slotsFound.length; i++) {
              slots[slotsFound[i].getAttribute("sf-insert-field")] =
                slotsFound[i];
            }

            // if sfUseDecorator is undefined the default decorator is used.
            var decorator = schemaFormDecorators.decorator(
              attrs.sfUseDecorator,
            );
            ...
            element[0].appendChild(
              sfBuilder.build(merged, decorator, slots, lookup),
            );
          ...
          scope.$on("schemaFormRedraw", function () {
            ...
            var form = scope.initialForm
              ? angular.copy(scope.initialForm)
              : ["*"];
            if (schema) {
              render(schema, form);
            }
    ...
    function (sfValidator, $parse, sfSelect) {
      return {
        ...
        link: function (scope, element, attrs, ngModel) {
          ...
          ["$parsers", "$viewChangeListeners", "$formatters"].forEach(
            function (attr) {
          ...
          scope.validateField = function (formName) {
            ...
            if (
              formName != undefined &&
              ngModel.$$parentForm.$name !== formName
            ) {
              return;
            }
